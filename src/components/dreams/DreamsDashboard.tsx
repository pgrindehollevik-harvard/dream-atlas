...
export function DreamsDashboard({ user, profile, initialDreams }: Props) {
  ...
  // Helper to parse rate limit errors and return brief message
  function parseRateLimitError(error: string): string {
    // Look for "try again in" pattern
    const tryAgainMatch = error.match(/try again in (\d+)\s*ms/i);
    if (tryAgainMatch) {
      const ms = parseInt(tryAgainMatch[1], 10);
      const seconds = Math.ceil(ms / 1000);
      return `Rate limit reached. Try again in ${seconds} second${seconds !== 1 ? "s" : ""}.`;
    }
  ...
  async function handleGenerateAggregate() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Show the actual error message if available, parsed for rate limits
        const errorMsg = json.details || json.error || "Could not generate themes.";
        setAggregateError(parseRateLimitError(errorMsg));
      } else {
  ...
  async function handleSendChat() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setChatError(parseRateLimitError(errorMsg));
  ...
  return (
    <main className="flex flex-1 flex-col gap-6 sm:gap-8 px-4 sm:px-0">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 self-start md:self-auto">
          <button
            ...
            className="rounded-full border border-slate-700 bg-night-900/50 px-4 py-2.5 text-xs font-medium text-slate-200 hover:border-dream-400 hover:bg-dream-500/10 hover:text-dream-300 active:scale-95 transition"
          >
          ...
          <button
            ...
            className="rounded-full border border-slate-700 bg-night-900/50 px-4 py-2.5 text-xs font-medium text-slate-200 hover:border-rose-400 hover:bg-rose-500/10 hover:text-rose-200 active:scale-95 transition"
          >
            Log out
          </button>
        </div>
      ...
      <section className="rounded-3xl border border-slate-800 bg-night-800/70 p-4 sm:p-5 shadow-xl">
        <div className="space-y-4 sm:space-y-3">
          ...
          {logOpen && (
            <div className="grid gap-4 sm:gap-3 md:grid-cols-[minmax(0,1.2fr),minmax(0,0.8fr)]">
              <div className="space-y-4 sm:space-y-3">
                <input
                  value={title}
              ...
              <div className="space-y-4 sm:space-y-3">
                ...
                <button
                  ...
                  className="mt-2 flex w-full items-center justify-center rounded-full bg-dream-500 px-4 py-3 text-xs font-medium text-night-900 shadow-glow transition hover:bg-dream-400 active:scale-95 disabled:cursor-not-allowed disabled:opacity-70"
                >
          ...
          {dreams.map((dream) => (
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2 px-4 pb-4 text-[11px]">
                <Link
                  href={`/d/${dream.slug}`}
                  className="rounded-full border border-slate-700 bg-night-900/50 px-3 py-1.5 text-[10px] font-medium text-slate-200 hover:border-dream-400 hover:bg-dream-500/10 hover:text-dream-300 active:scale-95 transition"
                >
                  Open
                </Link>
                <button
                  ...
                  className="rounded-full border border-slate-800 bg-night-900/50 px-3 py-1.5 text-[10px] font-medium text-slate-400 hover:border-rose-500/70 hover:bg-rose-500/10 hover:text-rose-300 active:scale-95 transition disabled:opacity-60 disabled:cursor-not-allowed"
                >
